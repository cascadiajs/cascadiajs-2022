...
module.exports = async function GetSpeakerData (req) {
  let speakerList = await data.get({ table: 'speakers', limit: 20 })
  // scrub info for non-revealed speakers (only include reveal & pixelated props)
  let speakers = speakerList.map(s => ((new Date(s.reveal)).getTime() <= Date.now() || s.placeholder ? s : { reveal: s.reveal, pixelated: s.pixelated, topics: s.topics }))
